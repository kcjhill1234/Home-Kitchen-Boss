document.addEventListener("DOMContentLoaded", async () => {
    ...
    const mapToOption = ({name, value}) => `<option value="${value}">${name}</option>`

    const dietJson = await (await fetch(getOptionsUrl('diet-choice'))).json();
    const cuisineJson = await (await fetch(getOptionsUrl('cuisine-choice'))).json()
    const intoleranceJson = await (await fetch(getOptionsUrl('intolerance-choice'))).json()
